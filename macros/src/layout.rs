...
use syn::{parse_macro_input, punctuated::Punctuated, DeriveInput, PathArguments, Token};
use syn::{Error, Ident, Type, Visibility};
    ...
    match field_type {
        Type::Path(type_path) => {
            ...
            let type_name_args = match type_name_str.as_str() {
                "Query" | "Section" | "Pane" => match &target.arguments {
...
pub fn macro_handler(layout: Layout, attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    }) = ast.data
    {
        ...
        return Error::new_spanned(struct_name, format!("#[panel] macro only supports structs"))
            .to_compile_error()
            .into();
    };
...
//                attr.path.is_ident("option") ||   // should be processed before and retain types
        ...
        let (_type_name_path, _type_name_generic) = get_type_info(&type_name);
    ...
    for field in fields.iter() {
        ...
        let ctl_args = field
            .args
            .get(&field.type_name_str_lower_case)
    ...
    let mut field_idents: Vec<Ident> = fields
        .iter()
        .map(|f| {
    ...
    let attrs_k: Vec<String> = attrs_kv
        .iter()
        .map(|item| {
            let v = item.0.clone();
