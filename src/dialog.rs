...
impl Button {
    ...
    pub fn name_and_class(&self) -> (String, Option<String>) {
        let (name, class) = match self {
            ...
            Self::__WithClass(name, class) => (name.as_str(), Some(class.clone())),
        };
...
impl Render for DialogButtonData {
    ...
    fn render_node(
        ...
        self.btn
            .render_with_class(parent, map, renderables, self.class)?;
...
pub fn show_dialog(title: &str, msg: &str) -> Result<Dialog> {
    let dialog = Dialog::new()?.set_title(title)?.set_msg(msg)?.show()?;
    Ok(dialog)
}

pub fn show_dialog_with_html(title: &str, msg: Html) -> Result<Dialog> {
    let dialog = Dialog::new()?.set_title(title)?.set_html_msg(msg)?.show()?;
    Ok(dialog)
}
