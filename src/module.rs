...
pub fn registry() -> Arc<RwLock<AHashMap<String,Arc<Module>>>> {
    ...
    if let Some(modules) = modules {
        ...
        unsafe {MODULES = Some(modules.clone());}
        modules
    }
}

pub async fn register(name:&str, iface: Arc<dyn ModuleInterface>, container_types : &[u32]) -> Result<()> {
    ...
    if registry().write()?.insert(name.to_string(), module.clone()).is_some() {
        panic!("Error: multiple registrations for module {}.  Modules are singletons.", name);
    }
...
pub fn data_types_to_modules() -> Result<Rc<RefCell<AHashMap<u32,Arc<Module>>>>> {
    ...
    match map {
        Some(map) => Ok(map),
        None => {
            Err(Error::DataTypesToModuleMapMissing)
        }
    }
}
...
pub fn get_module(name:&str) -> Option<Arc<Module>> {
    match registry().read().expect(&format!("Unable to locate module {}", name)).get(name) {
...
where T : Send + Sync + 'static
{
    ...
    match registry().read().expect("Unable to lock module registry").get(name) {
        Some(module) => {
