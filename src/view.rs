...
use crate::{app_menu::AppMenu, events, prelude::*};
...
impl Container {
    ...
    pub fn meta<M>(&self) -> Result<Arc<M>>
    where
        M: AnySync,
    {
        let view = self.view().ok_or("Unable to get current view")?;
...
pub fn get_meta<M>(view: Arc<dyn View>) -> Result<Arc<M>>
where
    M: AnySync,
{
    let meta_view = view.downcast_arc::<MetaView>()?;
...
pub struct MetaView {
    ...
    pub meta: Arc<dyn Meta>, // pub meta:Arc<dyn AnySync>
}
...
impl MetaView {
    ...
    pub fn try_new(
        view: Arc<dyn View>,
        meta: Arc<dyn Meta>, // meta: Arc<dyn AnySync>
    ) -> Result<Arc<dyn View>>
// where V: View
    {
        let view: Arc<dyn View> = Arc::new(Self { view, meta });
