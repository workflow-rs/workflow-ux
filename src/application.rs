...
impl Application {
    ...
    pub async fn load_modules(&self, pkg:JsValue, module_load_order : &[&str], module_disable_list : &[&str])->Result<JsValue>{
        ...
        let mut modules = AHashMap::<String,JsValue>::new(); //Vec::new();
        ...
        for idx in 0..keys_vec.len() {
            ...
            if name.starts_with("module_register_") {
                ...
                let name = name.replace("module_register_", "").replace("_wasm","");//.to_lowercase();

                modules.insert(name, keys_vec[idx].clone());
        ...
        for (name,module_load_fn_name) in modules.iter() {
