...
use workflow_html::{Render, ElementResult, Renderables, Hooks};
...
impl Badge {
    pub fn element(&self) -> FlowDataBadgeGraph {
        self.element.clone().dyn_into::<FlowDataBadgeGraph>().expect("Unable to cast element to FlowDataBadgeGraph")
    }
    ...
    pub fn create(title:&str, options: Options)->Result<Self> {
        ...
        */
        let element = document()
            .create_element("flow-data-badge-graph")?;
