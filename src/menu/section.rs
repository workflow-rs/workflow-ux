...
impl SectionMenu{
    ...
    pub fn select(&self) -> Result<()> {
        ...
        if let Some(sub_menu_container) = self.sub_menu_container.as_ref(){
            ...
            for idx in 0..els.length() {
                els.item(idx).unwrap().dyn_into::<Element>().unwrap().class_list().remove_1("active")?;
            }
    ...
    pub fn with_callback(mut self, callback: Box<dyn Fn(&SectionMenu) -> Result<()>>) -> Result<Self> {
